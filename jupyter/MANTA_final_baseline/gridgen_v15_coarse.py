...
def setGrid(steps=4, plot=True):
    ...
    com.geqdskfname[0] = 'FDC24_TokaMaker_2024-12-XX_v15_FS_truncate_false.geqdsk'
    com.aeqdskfname[0] = 'aeqdsk_v15.txt'
    ...
    if steps>1:
        ...
        com.nxcore[0] = [7,5]#[14,10] # No cells in inner/outer core region
        ...
        write_gridue("gridue_v15_orthogonal_coarse_1.hdf5") # Write HDF5 gridue
        c.reload() # Update Case arrays to reflect zshift
        if plot:
            f = c.plot.gridue('gridue_v15_orthogonal_coarse_1.hdf5', plates=True)
    ...
    if steps>3:
        ...
        grd.grdrun() # generate gridue file 
        write_gridue("gridue_v15_vertical_coarse_1.hdf5") # Write HDF5 gridue

        if plot:
            f = c.plot.gridue('gridue_v15_vertical_coarse_1.hdf5', plates=False)
            f.get_axes()[0].plot(grd.rplate1, grd.zplate1, 'r-')
            f.get_axes()[0].plot(grd.rplate2, grd.zplate2, 'b-')
