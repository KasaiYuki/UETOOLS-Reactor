...
def setGrid(steps=4, plot=True):
    ...
    com.geqdskfname[0] = 'FDC24_TokaMaker_2024-10-15_v7_FS_nt.geqdsk'
    com.aeqdskfname[0] = 'aeqdsk_v7'
    ...
    if steps>3:
        ...
        grd.grdrun() # generate gridue file 
        write_gridue("gridue_v7.hdf5") # Write HDF5 gridue

        if plot:
            f = c.plot.gridue('gridue_v7.hdf5', plates=False)
            f.get_axes()[0].plot(grd.rplate1, grd.zplate1, 'r-')
            f.get_axes()[0].plot(grd.rplate2, grd.zplate2, 'b-')
