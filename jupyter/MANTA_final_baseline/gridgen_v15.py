...
def setGrid(steps=4, plot=True):
    ...
    if steps>1:
        ...
        write_gridue("gridue_v12_orthogonal.hdf5") # Write HDF5 gridue
        c.reload() # Update Case arrays to reflect zshift
        if plot:
            f = c.plot.gridue('gridue_v12_orthogonal.hdf5', plates=True)
    ...
    if steps>3:
        ...
        grd.grdrun() # generate gridue file 
        write_gridue("gridue_v12_vertical.hdf5") # Write HDF5 gridue

        if plot:
            f = c.plot.gridue('gridue_v12_vertical.hdf5', plates=False)
            f.get_axes()[0].plot(grd.rplate1, grd.zplate1, 'r-')
            f.get_axes()[0].plot(grd.rplate2, grd.zplate2, 'b-')
